 ...
 */
  ...
  if(mem_alg == FIRST_FIT){
	...
	if(mem_alg == WORST_FIT){
      ...
      max_ptr = hole_head;
      max_prev = NIL_HOLE;
			/* check list of all block for the biggest hole */
      while (hp != NIL_HOLE && hp->h_base < swap_base) {
        ...
        prev_ptr = hp;
        hp = hp->h_next;
      } /* we found the biggest hole - cut off some memory for our process*/
      if(max_ptr->h_len >= clicks) { /* only do that if process asked for less than the size of biggest hole */
        old_base = max_ptr->h_base;
        max_ptr->h_base += clicks;
        max_ptr->h_len -= clicks;
        if(max_ptr->h_len == 0) del_slot(max_prev, max_ptr); /* delete slot if it is not a hole anymore */
        return(old_base);
      }
    } while (swap_out());
  }
 ...
 *===========================================================================*/
 PUBLIC int do_hole_map(void)
 {
   ...
   temp = hole_head;
   for(i = 0;  i < (int)((nbytes-sizeof(phys_clicks))/(2*sizeof(phys_clicks))), temp != NIL_HOLE; temp = temp->h_next, i++)
   {	 
	sys_copy(MM_PROC_NR, D, (phys_bytes)&temp->h_len,
	      mm_in.m_source, D, (phys_bytes)(buffer+2*i*sizeof(phys_clicks)), (phys_bytes)sizeof(phys_clicks));

    sys_copy(MM_PROC_NR, D, (phys_bytes)&temp->h_base,
	      mm_in.m_source, D, (phys_bytes)(buffer+(2*i+1)*sizeof(phys_clicks)), (phys_bytes)sizeof(phys_clicks));

   }
   sys_copy(MM_PROC_NR, D, (phys_bytes)&a, 
	    mm_in.m_source, D, (phys_bytes)(buffer+(2*(i+1)*sizeof(phys_clicks))), (phys_bytes)sizeof(phys_clicks));
 ...
 *===========================================================================*/
 PUBLIC int do_worst_fit(void)
 {
   if((mm_in.m1_i1 == FIRST_FIT) || (mm_in.m1_i1 == WORST_FIT)) mem_alg = mm_in.m1_i1;
   return 0;
 }
