...
FORWARD _PROTOTYPE( int do_setsched, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_getgroup, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_setgroup, (message *m_ptr) );
FORWARD _PROTOTYPE( int do_getsched, (message *m_ptr) );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_GETSCHED:  r = do_getsched(&m);	break;
...
/* funkcja zmieniajaca proporcje czasowa */
PRIVATE int do_setsched(m_ptr)
message *m_ptr;
{
	int new_quantum_percent;
	new_quantum_percent = m_ptr -> m1_i1;

	quantum_percent = new_quantum_percent;
	return 0;
}

/* funkcja zwracajaca proporcje */
PRIVATE int do_getsched(m_ptr)
message *m_ptr;
{
	return quantum_percent;
}

/* funkcja zmieniajaca grupe */
...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  /* Przydzielanie procesow do grup */
  if(rpc -> p_pid % 2 == 0) 
  	rpc -> group = GROUPA;
